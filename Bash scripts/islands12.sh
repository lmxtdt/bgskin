...
echo "islands12 start: 280,000 total, 80,000 iterations of each fitness" >> timeIsland.txt
date >> timeIsland.txt
	...
	do for b in 19 20 21
		do for iter in {0..9}
			do ../build/slim -d "replicate = $rep" -d "iter = $iter" \
					...
					-d "f = $f" -d "b = $b" -d "c = $c" \
					-d "outputPath = 'island12.csv'" -d "makeSeeds = 0" \
...
echo "islands12 end" >> timeIsland.txt
date >> timeIsland.txt