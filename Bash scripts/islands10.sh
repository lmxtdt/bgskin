...
echo "islands10 start: 280,000 total, 40,000 iterations of each fitness" >> timeIsland.txt
date >> timeIsland.txt
	...
	do for f in 0 -1000 -6 -8 -10 -12 -14
		do for iter in {0..9}
			do ../build/slim -d "replicate = $rep" -d "iter = $iter" \
					...
					-d "f = $f" -d "b = $b" -d "c = $c" \
					-d "outputPath = 'island10.csv'" -d "makeSeeds = 0" \
...
echo "islands10 end" >> timeIsland.txt
date >> timeIsland.txt