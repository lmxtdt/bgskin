...
#cost: 0.0 benefit: 0.19
let "c = 10"
let "b = 19"

echo "islands8 start: 560,000 total, 80,000 iterations of each fitness" >> timeIsland.txt
date >> timeIsland.txt
	...
	do for f in 0 -1000 -6 -8 10 -12 -14
		do for iter in {0..9}
			do ../build/slim -d "replicate = $rep" -d "iter = $iter" \
					...
					-d "f = $f" -d "b = $b" -d "c = $c" \
					-d "outputPath = 'island08.csv'" -d "makeSeeds = 0" \
...
echo "islands8 end" >> timeIsland.txt
date >> timeIsland.txt